...
// OllamaProvider implements the LLMProvider interface using Ollama API
...
// GenerateResponse implements LLMProvider interface
...
// GenerateResponseWithOptions implements LLMProvider interface
...
// CountTokens implements LLMProvider interface
...
func (p *OllamaProvider) verifyModelAvailability(autoPull bool) error {
	...
	// If the model is not found and autoPull is enabled, pull it
...
func (p *OllamaProvider) pullModel() error {
	...
	// Send request
	client := &http.Client{
		// Model pulls can take a very long time
